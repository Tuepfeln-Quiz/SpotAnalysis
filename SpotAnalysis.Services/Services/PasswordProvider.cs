...
public static class PasswordProvider
{
    ...
    {
        return
            $"$argon2id$v={Argon2Version}$m={ArgonMemory},t={ArgonIterations},p={ArgonParallelism}${Convert.ToBase64String(salt)}${Convert.ToBase64String(hash)}";
    }
    ...
    public sealed class Password
    {
        ...
        public static Password FromParamString(string paramString)
        {
            if (string.IsNullOrEmpty(paramString))
                throw new ArgumentException("Parameter string cannot be null or empty");
